...
На рисунке показан пример системы машинного зрения по распознаванию различных изображений на этикетках, либо поиска определенного паттерна на поверхности. IP камеры подключаются к **UberNAX AI Server** напрямую, либо через 10Гб/с Ethernet коммутатор. Щелевые датчики подключены к **UberNAX Sensor Brocker** с модулем цифрового ввода. Сетевое взаимодействие компонентов машинного зраения организовано через программный маршрутизатор - **UberNAX Soft router**, а сохранение обработанных данных выполнено на отдельном сетевом хранилище - **UberNAX NAS Storage**.
